...
template float cuVEC_VC<float>::sum_nonempty(size_t arr_size, cuBox box);
template double cuVEC_VC<double>::sum_nonempty(size_t arr_size, cuBox box);

template cuFLT3 cuVEC_VC<cuFLT3>::sum_nonempty(size_t arr_size, cuBox box);
template cuDBL3 cuVEC_VC<cuDBL3>::sum_nonempty(size_t arr_size, cuBox box);

template <typename VType>
__host__ VType cuVEC_VC<VType>::sum_nonempty(size_t arr_size, cuBox box)
{
	zero_aux_values <<<1, CUDATHREADS >>> (cuVEC<VType>::aux_value, cuVEC<VType>::aux_value2, cuVEC<VType>::aux_value3, cuVEC<VType>::aux_real, cuVEC<VType>::aux_real2, cuVEC<VType>::aux_integer);
...
template float cuVEC_VC<float>::sum_nonempty(size_t arr_size, cuRect rectangle);
template double cuVEC_VC<double>::sum_nonempty(size_t arr_size, cuRect rectangle);

template cuFLT3 cuVEC_VC<cuFLT3>::sum_nonempty(size_t arr_size, cuRect rectangle);
template cuDBL3 cuVEC_VC<cuDBL3>::sum_nonempty(size_t arr_size, cuRect rectangle);

template <typename VType>
__host__ VType cuVEC_VC<VType>::sum_nonempty(size_t arr_size, cuRect rectangle)
{
	//if empty rectangle then average ove the entire mesh
	if (rectangle.IsNull()) return sum_nonempty(arr_size, cuBox());
