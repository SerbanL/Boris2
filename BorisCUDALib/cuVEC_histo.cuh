...
template double cuVEC<cuReal3>::chunked_averagemag(cuINT3 macrocell_dims);
...
template <typename VType>
__host__ double cuVEC<VType>::chunked_averagemag(cuINT3 macrocell_dims)
{
	cuSZ3 n_cpu = size_cpu();

	//number of cells used to perform pre-averaging
	cuSZ3 num_av_cells = cu_round((cuReal3)n_cpu / macrocell_dims);

	//make sure there's enough memory allocated
	if (!allocate_histogram_memory(0, num_av_cells.dim())) return false;
	...
	//do final average on preaveraged data
	zero_aux_values <<<1, CUDATHREADS >>> (aux_value, aux_value2, aux_value3, aux_real, aux_real2, aux_integer);

	average_preaverage_kernel <<< (num_av_cells.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
		(histogram_preaverage, histogram_preaverage_size, aux_real, aux_integer);

	double av = get_gpu_value(aux_real);
