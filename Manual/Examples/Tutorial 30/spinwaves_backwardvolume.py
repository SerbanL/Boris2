...
#if this value is too high (2 nm) significant discrepancy between simulations and analytical lines results (Brillouin zone smaller)
...
#use this instead to excite even and odd modes (in this case you should also reduce the cellsize along width to 1 nm)
#ns.editstagevalue(0, 'H0, He * sinc(kc*(x-Lx/2))*sinc(2*PI*fc*(t-t0))*sum(i;1;25;sin(<i>*PI*y/50e-9)), 0')
...
plt.axes(xlabel = 'k (rad/m)', ylabel = 'f (Hz)', title = 'Backward Volume')
...
plt.savefig('backwardvolume.png', dpi = 600)
plt.show()
