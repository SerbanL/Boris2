...
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_EnergyChange_SC_DemagCUDA(int spin_index, cuReal3 Mnew)
{
	if (pAtom_Demag_Heff && pAtom_Demag_Heff->linear_size()) {
		...
		return -(cuBReal)MUB_MU0 * (*pAtom_Demag_Heff)[M1.cellidx_to_position(spin_index)] * (Mnew - M1[spin_index]);
	}
	else return 0.0;
}
...
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_EnergyChange_SC_SurfExchangeCUDA(int spin_index, cuReal3 Mnew)
{
	...
	if (spin_index / (n.x * n.y) == n.z - 1 && paMesh_Top_size) {
	...
	if (spin_index / (n.x * n.y) == 0 && paMesh_Bot_size) {
...
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC(int spin_index, int*& cuaModules, int numModules, cuReal3& Ha)
{
	...
	for (int idx = 0; idx < numModules; idx++) {
		...
		case MOD_DEMAG_N:
			energy += Get_Atomistic_Energy_SC_DemagNCUDA(spin_index);
			break;

		case MOD_DEMAG:
			energy += Get_Atomistic_Energy_SC_DemagCUDA(spin_index);
			break;

		case MOD_ATOM_DIPOLEDIPOLE:
			energy += Get_Atomistic_Energy_SC_DipoleDipoleCUDA(spin_index);
			break;
		...
		case MOD_SURFEXCHANGE:
			energy += Get_Atomistic_Energy_SC_SurfExchangeCUDA(spin_index);
			break;
		...
		case MOD_ANIBI:
			energy += Get_Atomistic_Energy_SC_AnisotropyBiaxialCUDA(spin_index);
			break;

		case MOD_ANITENS:
			energy += Get_Atomistic_Energy_SC_AnisotropyTensorialCUDA(spin_index);
			break;
...
//Atom_Demag_N
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC_DemagNCUDA(int spin_index)
{
	cuVEC_VC<cuReal3>& M1 = *pM1;
	...
	return ((cuBReal)MUB_MU0 / 2) * r * (Nxy.x * S.x*S.x + Nxy.y * S.y*S.y + Nz * S.z*S.z);
}

//Atom_Demag
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC_DemagCUDA(int spin_index)
{
	if (pAtom_Demag_Heff && pAtom_Demag_Heff->linear_size()) {

		cuVEC_VC<cuReal3>& M1 = *pM1;

		return -(cuBReal)MUB_MU0 * (*pAtom_Demag_Heff)[M1.cellidx_to_position(spin_index)] * M1[spin_index];
	}
	else return 0.0;
}

//Atom_DipoleDipole
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC_DipoleDipoleCUDA(int spin_index)
{
	if (pAtom_Demag_Heff && pAtom_Demag_Heff->linear_size()) {
...
//Atom_SurfExchangeCUDA
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC_SurfExchangeCUDA(int spin_index)
{
	cuBReal energy_ = 0;
...
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC_AnisotropyCUDA(int spin_index)
{
	...
	//Hamiltonian contribution as -Ku * (S * ea)^2, where S is the local spin direction
	return -K1 * dotprod * dotprod - K2 * dpsq * dpsq;
}
...
//Atom_AnisotropyBiaxialCUDA
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC_AnisotropyBiaxialCUDA(int spin_index)
{
	cuVEC_VC<cuReal3>& M1 = *pM1;
...
//Atom_AnisotropyTensorialCUDA
__device__ cuBReal ManagedAtom_MeshCUDA::Get_Atomistic_Energy_SC_AnisotropyTensorialCUDA(int spin_index)
{
	cuVEC_VC<cuReal3>& M1 = *pM1;
