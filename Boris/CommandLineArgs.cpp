...
enum CLARGS_ {
	...
	CLARGS_PYSCRIPT_MGPU,

	CLARGS_PYSCRIPT_PARALLELSPLIT,

	CLARGS_PYSCRIPT_TERMINATE,
...
void CLArgs::process_command_line_args(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if (arg == "-p") {
			...
			clargs.push_back(CLARGS_WINDOW);
			argnum.push_back(i);
		}
		else if (arg == "-s") {
			...
			clargs.push_back(CLARGS_PYSCRIPT_MGPU);
			argnum.push_back(i);
		}
		else if (arg == "-sp") {
			...
			clargs.push_back(CLARGS_PYSCRIPT_TERMINATE);
			argnum.push_back(i);
		}
		else if (arg == "-sd") {
	...
	for (int idx = 0; idx < clargs.size(); idx++) {
		...
		case CLARGS_WINDOW:
			if (clargs_fields[idx].size() >= 1) window_startup_option = clargs_fields[idx][0];
			break;
		...
		case CLARGS_PYSCRIPT_MGPU:
			if (clargs_fields[idx].size() >= 1) {

				std::stringstream ss(clargs_fields[idx][0]);
				ss >> python_script_mGPU;
			}
			break;
		...
		case CLARGS_PYSCRIPT_TERMINATE:
			if (clargs_fields[idx].size() >= 1) {

				std::stringstream ss(clargs_fields[idx][0]);
				ss >> python_script_terminate;
			}
			break;
