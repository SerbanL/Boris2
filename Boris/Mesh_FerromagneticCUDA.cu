...
DBL3 FMeshCUDA::Average_dmdt(cuBox avBox)
{
	Zero_aux_values();

	Average_dmdt_FM_kernel <<< (avBox.size().dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (avBox, cuMesh, Get_ManagedDiffEqCUDA(), aux_real3, aux_int);
...
DBL3 FMeshCUDA::Average_mxdmdt(cuBox avBox)
{
	Zero_aux_values();

	Average_mxdmdt_FM_kernel <<< (avBox.size().dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (avBox, cuMesh, Get_ManagedDiffEqCUDA(), aux_real3, aux_int);
