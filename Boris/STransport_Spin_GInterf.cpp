...
double STransport::Afunc_V(int cell1_idx, int cell2_idx, DBL3 relpos_m1, DBL3 shift, DBL3 stencil, TransportBase& trans_sec, TransportBase& trans_pri) const
{
	...
	if (!trans_pri.pMeshBase->is_atomistic() && !trans_sec.pMeshBase->is_atomistic()) {

		//primary and secondary meshes
		Mesh* pMesh_pri = reinterpret_cast<Mesh*>(trans_pri.pMeshBase);
		Mesh* pMesh_sec = reinterpret_cast<Mesh*>(trans_sec.pMeshBase);
		...
		if (trans_pri.Get_STSolveType() == STSOLVE_FERROMAGNETIC) {
