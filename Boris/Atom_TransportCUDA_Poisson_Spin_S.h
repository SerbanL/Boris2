...
class Atom_TransportCUDA_Spin_S_Funcs {
...
public:
	...
	ManagedAtom_DiffEqCubicCUDA* pcuDiffEq;

	//need this (held in TransportCUDA) to access delsq V (Poisson_RHS) calculation
	Atom_TransportCUDA_Spin_V_Funcs* pPoisson_Spin_V;
...
public:
	...
	BError set_pointers(Atom_MeshCUDA* paMeshCUDA, Atom_DifferentialEquationCubicCUDA* pdiffEqCUDA, Atom_TransportCUDA* pTransportCUDA);
	...
	__device__ cuReal3 Poisson_RHS(int idx)
	{
		...
		/*
		cuBReal l_sf = *pcuaMesh->pl_sf;
		pcuaMesh->update_parameters_ecoarse(idx, *pcuaMesh->pl_sf, l_sf);
		...
		if (stsolve == STSOLVE_FERROMAGNETIC) {
			...
			cuBReal l_ex = *pcuaMesh->pl_ex;
			cuBReal l_ph = *pcuaMesh->pl_ph;
			pcuaMesh->update_parameters_ecoarse(idx, *pcuaMesh->pMs, Ms, *pcuaMesh->pl_ex, l_ex, *pcuaMesh->pl_ph, l_ph);
	...
	__device__ cuReal3 a_func_pri(int cell1_idx, int cell2_idx, cuReal3 shift)
	{
		...
		if (stsolve == STSOLVE_FERROMAGNETIC) {
			...
			pcuaMesh->update_parameters_ecoarse(cell1_idx, *pcuaMesh->pMs, Ms, *pcuaMesh->pDe, De, *pcuaMesh->pP, P);
			...
			bool cpump_enabled = cuIsNZ(pcuaMesh->pcpump_eff->get0());
			bool the_enabled = cuIsNZ(pcuaMesh->pthe_eff->get0());
			...
			if (cuIsZ(shift.z) && (cpump_enabled || the_enabled)) {
				...
				if (the_enabled) {
					...
					pcuaMesh->update_parameters_ecoarse(cell1_idx, *pcuaMesh->pn_density, n_density);

					cuReal3 B1 = (grad_m1.x ^ grad_m1.y);
					a1 += pcuaMesh->pthe_eff->get0() * ((cuBReal)HBAR_E * (cuBReal)MUB_E * sigma_1 * sigma_1 / ((cuBReal)ECHARGE * n_density)) * ((-E1.y * B1 * u.x) + (E1.x * B1 * u.y));

					cuReal3 B2 = (grad_m2.x ^ grad_m2.y);
					a2 += pcuaMesh->pthe_eff->get0() * ((cuBReal)HBAR_E * (cuBReal)MUB_E * sigma_2 * sigma_2 / ((cuBReal)ECHARGE * n_density)) * ((-E2.y * B2 * u.x) + (E2.x * B2 * u.y));
				}
				...
				if (cpump_enabled) {
					...
					a1 += pcuaMesh->pcpump_eff->get0() * ((cuBReal)HBAR_E * (cuBReal)MUB_E * sigma_1 / 2) * (((dm_dt_1 ^ grad_m1.x) * u.x) + ((dm_dt_1 ^ grad_m1.y) * u.y));

					cuReal3 dm_dt_2 = (*pdM_dt)[idx_M2] / Ms;
					a2 += pcuaMesh->pcpump_eff->get0() * ((cuBReal)HBAR_E * (cuBReal)MUB_E * sigma_2 / 2) * (((dm_dt_2 ^ grad_m2.x) * u.x) + ((dm_dt_2 ^ grad_m2.y) * u.y));
				}
			}
	...
	__device__ cuReal3 a_func_sec(cuReal3 relpos_m1, cuReal3 shift, cuReal3 stencil)
	{
		...
		if (stsolve == STSOLVE_FERROMAGNETIC) {
			...
			cuBReal De = *pcuaMesh->pDe;
			cuBReal P = *pcuaMesh->pP;
			pcuaMesh->update_parameters_atposition(relpos_m1, *pcuaMesh->pMs, Ms, *pcuaMesh->pDe, De, *pcuaMesh->pP, P);
			...
			if (cuIsZ(shift.z) && (cpump_enabled || the_enabled)) {
				...
				if (the_enabled) {
					...
					cuBReal n_density = *pcuaMesh->pn_density;
					pcuaMesh->update_parameters_atposition(relpos_m1, *pcuaMesh->pn_density, n_density);
	...
	{
		cuBReal De = *pcuaMesh->pDe;
		pcuaMesh->update_parameters_ecoarse(cell1_idx, *pcuaMesh->pDe, De);
	...
	{
		cuBReal De = *pcuaMesh->pDe;
		pcuaMesh->update_parameters_atposition(relpos_m1, *pcuaMesh->pDe, De);
	...
	__device__ cuBReal c_func_pri(int cell_idx)
	{
		...
		cuBReal De = *pcuaMesh->pDe;
		pcuaMesh->update_parameters_ecoarse(cell_idx, *pcuaMesh->pDe, De);
	...
	__device__ cuBReal c_func_sec(cuReal3 relpos, cuReal3 stencil)
	{
		...
		cuBReal De = *pcuaMesh->pDe;
		pcuaMesh->update_parameters_atposition(relpos, *pcuaMesh->pDe, De);
