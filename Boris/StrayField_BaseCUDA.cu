...
__global__ void CalculateStrayField_kernel(cuVEC<cuReal3>& strayField, ManagedMeshCUDA* Mdipoles, size_t num_dipoles)
{
	...
	if (cell_idx < strayField.linear_size()) {
		...
		for (int idx = 0; idx < num_dipoles; idx++) {
			...
			cuRect dipoleRect = Mdipoles[idx].pM->rect;
...
void StrayField_BaseCUDA::CalculateStrayField(void)
{
	if (!dipoles.size()) strayField()->set(cuReal3(0.0));
		...
		CalculateStrayField_kernel <<< (strayField_size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (strayField, managed_dipoles, managed_dipoles.size());
	}
}
