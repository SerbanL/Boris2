...
__device__ cuReal2 ManagedMeshCUDA::Get_EnergyChange_AFM_ExchangeCUDA(int spin_index, cuReal3 Mnew_A, cuReal3 Mnew_B)
{
	...
	cuBReal energyA = -MU0 * M[spin_index] * Hexch / 2;
	cuBReal energyB = -MU0 * M2[spin_index] * Hexch2 / 2;
	...
	if (Mnew_A != cuReal3() && Mnew_B != cuReal3()) {
		...
		delsq_M_A = M.delsq_neu(spin_index);
		delsq_M_B = M2.delsq_neu(spin_index);

		Hexch = (2 * A_AFM.i / (MU0*Ms_AFM.i*Ms_AFM.i)) * delsq_M_A + (4 * Ah.i * M2[spin_index] + Anh.i * delsq_M_B) / (MU0*Ms_AFM.i*Ms_AFM.j);
		Hexch2 = (2 * A_AFM.j / (MU0*Ms_AFM.j*Ms_AFM.j)) * delsq_M_B + (4 * Ah.j * M[spin_index] + Anh.j * delsq_M_A) / (MU0*Ms_AFM.i*Ms_AFM.j);

		cuBReal energynewA = -MU0 * M[spin_index] * Hexch / 2;
		M[spin_index] = Mold_A;

		cuBReal energynewB = -MU0 * M2[spin_index] * Hexch2 / 2;
		M2[spin_index] = Mold_B;

		//multiply by 2 as we are not double-counting here (same as for Demag)
		return M.h.dim() * cuReal2(energynewA - energyA, energynewB - energyB) * 2;
	}
	//If Mnew is null then this method is used to obtain current energy only, not energy change
	else return M.h.dim() * cuReal2(energyA, energyB) * 2;
}
