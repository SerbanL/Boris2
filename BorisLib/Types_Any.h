...
//
// Any g;
// g.convert_string_set_type("8000", typeid(double).name());	//stores 8000 as a double - use this when type of Any has not been set yet, or to overwrite set type.
// g.clear();			//clear stored type and value
// g.IsNull();			//anything stored?
// g.get_type();		//get typeid name
...
private:
	...
	template <typename RType, typename Type> RType RunThisMethod(ConvertFromStringComponents_Params param)
	{
		...
		for (auto it = ctor_parameters.rbegin(); it != ctor_parameters.rend(); ++it) {
			...
			if (available_parameters >= *it) {
				...
				*static_cast<Type*>(pValue) = (Type)ToNum(convert_text, param.unit);
	...
	template <typename RType, typename PType> RType MatchType_CallFunc(PType param) {
		...
		else if (type_name == typeid(SEQP).name()) {

			return RunThisMethod<RType, SEQP>(param);
		}

		else if (type_name == typeid(COSSEQ).name()) {

			return RunThisMethod<RType, COSSEQ>(param);
		}

		else if (type_name == typeid(COSSEQ3).name()) {

			return RunThisMethod<RType, COSSEQ3>(param);
		}

		else if (type_name == typeid(SINOSC).name()) {

			return RunThisMethod<RType, SINOSC>(param);
		}

		else if (type_name == typeid(COSOSC).name()) {

			return RunThisMethod<RType, COSOSC>(param);
		}

		else if (type_name == typeid(SINOSC3).name()) {

			return RunThisMethod<RType, SINOSC3>(param);
		}

		else if (type_name == typeid(COSOSC3).name()) {
...
public:
	...
	bool IsNull(void) { return (pValue == nullptr); }

	bool is_type(const type_info& ti) { return (type_name == ti.name()); }
