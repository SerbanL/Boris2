...
#define NEWTONRAPHSON_TIMEOUT	20
...
inline double Root_NewtonRaphson(std::function<double(double)>& F, std::function<double(double)>& Fdiff, double x0, double accuracy)
{
	//starting values
	double xn = x0;
	double yn = F(x0);
	...
	while (fabs(yn) > accuracy && iterations < NEWTONRAPHSON_TIMEOUT) {
