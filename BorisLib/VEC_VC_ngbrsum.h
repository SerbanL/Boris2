...
VType VEC_VC<VType>::ngbr_dirsum(int idx) const
{
	...
	if ((ngbrFlags[idx] & NF_BOTHX) == NF_BOTHX) {
		...
		//both neighbors available so must be an inner point along this direction
		sum = (quantity[idx + 1] / GetMagnitude(quantity[idx + 1]) + quantity[idx - 1] / GetMagnitude(quantity[idx - 1]));
	}
	else if (ngbrFlags[idx] & NF_NGBRX) {
		...
		if (ngbrFlags[idx] & NF_NPX) {
			...
			if (ngbrFlags[idx] & NF_PBCX) {
				...
				sum = (quantity[idx + 1] / GetMagnitude(quantity[idx + 1]) + quantity[idx + n.x - 1] / GetMagnitude(quantity[idx + n.x - 1]));
			}
			else {

				sum = quantity[idx + 1] / GetMagnitude(quantity[idx + 1]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCX) {
				...
				sum = (quantity[idx - (n.x - 1)] / GetMagnitude(quantity[idx - (n.x - 1)]) + quantity[idx - 1] / GetMagnitude(quantity[idx - 1]));
			}
			else {
	...
	if ((ngbrFlags[idx] & NF_BOTHY) == NF_BOTHY) {
		...
		sum += (quantity[idx + n.x] / GetMagnitude(quantity[idx + n.x]) + quantity[idx - n.x] / GetMagnitude(quantity[idx - n.x]));
	}
	else if (ngbrFlags[idx] & NF_NGBRY) {
		...
		if (ngbrFlags[idx] & NF_NPY) {
			...
			if (ngbrFlags[idx] & NF_PBCY) {
				...
				sum += (quantity[idx + n.x] / GetMagnitude(quantity[idx + n.x]) + quantity[idx + (n.y - 1)*n.x] / GetMagnitude(quantity[idx + (n.y - 1)*n.x]));
			}
			else {

				sum += quantity[idx + n.x] / GetMagnitude(quantity[idx + n.x]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCY) {
				...
				sum += (quantity[idx - (n.y - 1)*n.x] / GetMagnitude(quantity[idx - (n.y - 1)*n.x]) + quantity[idx - n.x] / GetMagnitude(quantity[idx - n.x]));
			}
			else {
	...
	if ((ngbrFlags[idx] & NF_BOTHZ) == NF_BOTHZ) {
		...
		sum += (quantity[idx + n.x*n.y] / GetMagnitude(quantity[idx + n.x*n.y]) + quantity[idx - n.x*n.y] / GetMagnitude(quantity[idx - n.x*n.y]));
	}
	else if (ngbrFlags[idx] & NF_NGBRZ) {
		...
		if (ngbrFlags[idx] & NF_NPZ) {
			...
			if (ngbrFlags[idx] & NF_PBCZ) {
				...
				sum += (quantity[idx + n.x*n.y] / GetMagnitude(quantity[idx + n.x*n.y]) + quantity[idx + (n.z - 1) * n.x*n.y] / GetMagnitude(quantity[idx + (n.z - 1) * n.x*n.y]));
			}
			else {

				sum += quantity[idx + n.x*n.y] / GetMagnitude(quantity[idx + n.x*n.y]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCZ) {
				...
				sum += (quantity[idx - (n.z - 1) * n.x*n.y] / GetMagnitude(quantity[idx - (n.z - 1) * n.x*n.y]) + quantity[idx - n.x*n.y] / GetMagnitude(quantity[idx - n.x*n.y]));
			}
			else {
...
VType VEC_VC<VType>::anisotropic_ngbr_dirsum(int idx) const
{
	...
	if ((ngbrFlags[idx] & NF_BOTHX) == NF_BOTHX) {
		...
		//both neighbors available so must be an inner point along this direction
		sum = VType(0.0, -quantity[idx + 1].z, quantity[idx + 1].y) / GetMagnitude(quantity[idx + 1]) - VType(0.0, -quantity[idx - 1].z, quantity[idx - 1].y) / GetMagnitude(quantity[idx - 1]);
	}
	else if (ngbrFlags[idx] & NF_NGBRX) {
		...
		if (ngbrFlags[idx] & NF_NPX) {
			...
			if (ngbrFlags[idx] & NF_PBCX) {
				...
				sum = VType(0.0, -quantity[idx + 1].z, quantity[idx + 1].y) / GetMagnitude(quantity[idx + 1]) - VType(0.0, -quantity[idx + n.x - 1].z, quantity[idx + n.x - 1].y) / GetMagnitude(quantity[idx + n.x - 1]);
			}
			else {

				sum = VType(0.0, -quantity[idx + 1].z, quantity[idx + 1].y) / GetMagnitude(quantity[idx + 1]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCX) {
				...
				sum = VType(0.0, -quantity[idx - (n.x - 1)].z, quantity[idx - (n.x - 1)].y) / GetMagnitude(quantity[idx - (n.x - 1)]) - VType(0.0, -quantity[idx - 1].z, quantity[idx - 1].y) / GetMagnitude(quantity[idx - 1]);
			}
			else {
	...
	if ((ngbrFlags[idx] & NF_BOTHY) == NF_BOTHY) {
		...
		sum += VType(quantity[idx + n.x].z, 0.0, -quantity[idx + n.x].x) / GetMagnitude(quantity[idx + n.x]) - VType(quantity[idx - n.x].z, 0.0, -quantity[idx - n.x].x) / GetMagnitude(quantity[idx - n.x]);
	}
	else if (ngbrFlags[idx] & NF_NGBRY) {
		...
		if (ngbrFlags[idx] & NF_NPY) {
			...
			if (ngbrFlags[idx] & NF_PBCY) {
				...
				sum += VType(quantity[idx + n.x].z, 0.0, -quantity[idx + n.x].x) / GetMagnitude(quantity[idx + n.x]) - VType(quantity[idx + (n.y - 1)*n.x].z, 0.0, -quantity[idx + (n.y - 1)*n.x].x) / GetMagnitude(quantity[idx + (n.y - 1)*n.x]);
			}
			else {

				sum += VType(quantity[idx + n.x].z, 0.0, -quantity[idx + n.x].x) / GetMagnitude(quantity[idx + n.x]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCY) {
				...
				sum += VType(quantity[idx - (n.y - 1)*n.x].z, 0.0, -quantity[idx - (n.y - 1)*n.x].x) / GetMagnitude(quantity[idx - (n.y - 1)*n.x]) - VType(quantity[idx - n.x].z, 0.0, -quantity[idx - n.x].x) / GetMagnitude(quantity[idx - n.x]);
			}
			else {
	...
	if ((ngbrFlags[idx] & NF_BOTHZ) == NF_BOTHZ) {
		...
		sum += VType(-quantity[idx + n.x*n.y].y, quantity[idx + n.x*n.y].x, 0.0) / GetMagnitude(quantity[idx + n.x*n.y]) - VType(-quantity[idx - n.x*n.y].y, quantity[idx - n.x*n.y].x, 0.0) / GetMagnitude(quantity[idx - n.x*n.y]);
	}
	else if (ngbrFlags[idx] & NF_NGBRZ) {
		...
		if (ngbrFlags[idx] & NF_NPZ) {
			...
			if (ngbrFlags[idx] & NF_PBCZ) {
				...
				sum += VType(-quantity[idx + n.x*n.y].y, quantity[idx + n.x*n.y].x, 0.0) / GetMagnitude(quantity[idx + n.x*n.y]) - VType(-quantity[idx + (n.z - 1) * n.x*n.y].y, quantity[idx + (n.z - 1) * n.x*n.y].x, 0.0) / GetMagnitude(quantity[idx + (n.z - 1) * n.x*n.y]);
			}
			else {

				sum += VType(-quantity[idx + n.x*n.y].y, quantity[idx + n.x*n.y].x, 0.0) / GetMagnitude(quantity[idx + n.x*n.y]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCZ) {
				...
				sum += VType(-quantity[idx - (n.z - 1) * n.x*n.y].y, quantity[idx - (n.z - 1) * n.x*n.y].x, 0.0) / GetMagnitude(quantity[idx - (n.z - 1) * n.x*n.y]) - VType(-quantity[idx - n.x*n.y].y, quantity[idx - n.x*n.y].x, 0.0) / GetMagnitude(quantity[idx - n.x*n.y]);
			}
			else {
...
VType VEC_VC<VType>::zanisotropic_ngbr_dirsum(int idx) const
{
	...
	if ((ngbrFlags[idx] & NF_BOTHX) == NF_BOTHX) {
		...
		//both neighbors available so must be an inner point along this direction
		sum = VType(-quantity[idx + 1].z, 0.0, quantity[idx + 1].x) / GetMagnitude(quantity[idx + 1]) - VType(-quantity[idx - 1].z, 0.0, quantity[idx - 1].x) / GetMagnitude(quantity[idx - 1]);
	}
	else if (ngbrFlags[idx] & NF_NGBRX) {
		...
		if (ngbrFlags[idx] & NF_NPX) {
			...
			if (ngbrFlags[idx] & NF_PBCX) {
				...
				sum = VType(-quantity[idx + 1].z, 0.0, quantity[idx + 1].x) / GetMagnitude(quantity[idx + 1]) - VType(-quantity[idx + n.x - 1].z, 0.0, quantity[idx + n.x - 1].x) / GetMagnitude(quantity[idx + n.x - 1]);
			}
			else {

				sum = VType(-quantity[idx + 1].z, 0.0, quantity[idx + 1].x) / GetMagnitude(quantity[idx + 1]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCX) {
				...
				sum = VType(-quantity[idx - (n.x - 1)].z, 0.0, quantity[idx - (n.x - 1)].x) / GetMagnitude(quantity[idx - (n.x - 1)]) - VType(-quantity[idx - 1].z, 0.0, quantity[idx - 1].x) / GetMagnitude(quantity[idx - 1]);
			}
			else {
	...
	if ((ngbrFlags[idx] & NF_BOTHY) == NF_BOTHY) {
		...
		sum += VType(0.0, -quantity[idx + n.x].z, quantity[idx + n.x].y) / GetMagnitude(quantity[idx + n.x]) - VType(0.0, -quantity[idx - n.x].z, quantity[idx - n.x].y) / GetMagnitude(quantity[idx - n.x]);
	}
	else if (ngbrFlags[idx] & NF_NGBRY) {
		...
		if (ngbrFlags[idx] & NF_NPY) {
			...
			if (ngbrFlags[idx] & NF_PBCY) {
				...
				sum += VType(0.0, -quantity[idx + n.x].z, quantity[idx + n.x].y) / GetMagnitude(quantity[idx + n.x]) - VType(0.0, -quantity[idx + (n.y - 1)*n.x].z, quantity[idx + (n.y - 1)*n.x].y) / GetMagnitude(quantity[idx + (n.y - 1)*n.x]);
			}
			else {

				sum += VType(0.0, -quantity[idx + n.x].z, quantity[idx + n.x].y) / GetMagnitude(quantity[idx + n.x]);
			}
		}
		else {
			...
			if (ngbrFlags[idx] & NF_PBCY) {
				...
				sum += VType(0.0, -quantity[idx - (n.y - 1)*n.x].z, quantity[idx - (n.y - 1)*n.x].y) / GetMagnitude(quantity[idx - (n.y - 1)*n.x]) - VType(0.0, -quantity[idx - n.x].z, quantity[idx - n.x].y) / GetMagnitude(quantity[idx - n.x]);
			}
			else {
