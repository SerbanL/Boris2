...
//open a file programatically : the file name includes a path. Pass in any command line arguments as const char*.
template<typename ... Args>
void open_file(std::string fileName, Args... args)
{
	pid_t pid = fork();
    if (pid == 0) {
...
{
	//On Linux make the user documents directory same as the program executable directory
	//This means data directory, simulations directory, BorisPythonScripts directory will be in executable directory
