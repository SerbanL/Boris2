...
template double VEC<FLT3>::chunked_averagemag(INT3 macrocell_dims);
template double VEC<DBL3>::chunked_averagemag(INT3 macrocell_dims);
...
template <typename VType>
double VEC<VType>::chunked_averagemag(INT3 macrocell_dims)
{
	//number of cells used to perform pre-averaging
	SZ3 num_av_cells = round((DBL3)n / macrocell_dims);

	magnitude_reduction.new_average_reduction();
...
#pragma omp parallel for
		for (int j_seg = 0; j_seg < num_av_cells.j; j_seg++) {
			for (int i_seg = 0; i_seg < num_av_cells.i; i_seg++) {
				...
				if (!average.IsNull()) magnitude_reduction.reduce_average(GetMagnitude(average));
			}
		}
	}

	return magnitude_reduction.average();
}