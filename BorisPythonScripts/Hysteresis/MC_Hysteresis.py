...
def atomistic_hysteresis_MC(
        outputFile,
        meshRect_nm, unitcell_nm, macrocell_nm,
        optionalCode_start, optionalCode_steps,
        Ms0 = 800e3, A = 13e-12, K1 = 10e3, Kea1 = [1, 0, 0],
        Temperature = 0.0,
        Hmax = 1e5, Hsteps = 100, H_angles_deg = [90, 0], cuda = True, updown_sweeps = False):
    
    """
    Simulate a simple hysteresis loop, saved in outputFile, for given mesh dimensions (meshRect_nm), with given unitcell_nm. Compute dipole-dipole interaction in macrocell_nm.
    Use atomistic Monte Carlo solver.
    ...
    Pass in required micromagnetic parameters, which will be converted to atomistic parameters.
    ...
    ns.setameshcubic(meshName, np.array(meshRect_nm) * 1e-9)
    ns.cellsize(np.array(unitcell_nm) * 1e-9)
    
    ns.addmodule(meshName, 'dipoledipole')
    ns.dmcellsize(np.array(macrocell_nm) * 1e-9)
    
    mu_s = unitcell_nm[0]*unitcell_nm[1]*unitcell_nm[2]*(1e-27/muB) * Ms0
    ns.setparam(meshName, 'mu_s', mu_s)
    
    J = 2*unitcell_nm[0]*1e-9 * A
    ns.setparam(meshName, 'J', J)
    
    K = K1*unitcell_nm[0]*1e-9**3
    ns.setparam(meshName, 'K1', K)
    ns.setparam(meshName, 'ea1', Kea1)
    if K1 != 0.0: ns.addmodule(meshName, 'aniuni')
    ...
    plt.plot(np.array(data[0]) / 1e3, np.array(data[1]) / 1e3, '.--', label = 'MC')
    plt.legend()
    plt.show()
