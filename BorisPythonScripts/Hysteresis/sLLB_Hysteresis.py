...
def micromagnetic_hysteresis_sLLB(
        outputFile,
        ...
        Hmax = 1e5, Hsteps = 100, H_angles_deg = [90, 0], mxh = 1e-4, dT = 25e-15, cuda = True, updown_sweeps = False):
    
    """
    Simulate a simple hysteresis loop, saved in outputFile, for given mesh dimensions (meshRect_nm), with given cellsize_nm.
    Use stochastic LLB equation, solved using TEuler with fixed dT.
    The user can inject custom configuration code through optionalCode function, which has the form def optionalCode_start(ns).
    ...
    ns.editstagestop(-1, 'mxh_iter', [mxh, 100000])
    ...
    plt.plot(np.array(data[0]) / 1e3, np.array(data[1]) / 1e3, '.--', label = 'sLLB')
    plt.legend()
    plt.show()
