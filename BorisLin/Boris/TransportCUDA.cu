...
__global__ void CalculateElectricalConductivity_TAMR_Equation_Kernel(ManagedMeshCUDA& cuMesh, ManagedFunctionCUDA<cuBReal, cuBReal, cuBReal, cuBReal>& TAMR_conductivity_equation)
{
	cuVEC_VC<cuBReal>& elC = *cuMesh.pelC;
	cuVEC_VC<cuReal3>& M = *cuMesh.pM;
...
void TransportCUDA::CalculateElectricalConductivity_TAMR(void)
{
	if (TAMR_conductivity_equation.is_set()) {
		...
		CalculateElectricalConductivity_TAMR_Equation_Kernel <<< (pMeshCUDA->n_e.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (pMeshCUDA->cuMesh, TAMR_conductivity_equation.get_x());
	}
	else {

		//default formula set
		CalculateElectricalConductivity_TAMR_Kernel <<< (pMeshCUDA->n_e.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (pMeshCUDA->cuMesh);
	}
}
...
__global__ void CalculateElectricalConductivity_TAMR_Equation_and_AMR_Kernel(ManagedMeshCUDA& cuMesh, ManagedFunctionCUDA<cuBReal, cuBReal, cuBReal, cuBReal>& TAMR_conductivity_equation)
{
	cuVEC_VC<cuBReal>& elC = *cuMesh.pelC;
	cuVEC<cuReal3>& E = *cuMesh.pE;
	cuVEC_VC<cuReal3>& M = *cuMesh.pM;
...
void TransportCUDA::CalculateElectricalConductivity_TAMR_and_AMR(void)
{
	if (TAMR_conductivity_equation.is_set()) {
		...
		CalculateElectricalConductivity_TAMR_Equation_and_AMR_Kernel <<< (pMeshCUDA->n_e.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (pMeshCUDA->cuMesh, TAMR_conductivity_equation.get_x());
	}
	else {

		//default formula set
		CalculateElectricalConductivity_TAMR_and_AMR_Kernel <<< (pMeshCUDA->n_e.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (pMeshCUDA->cuMesh);
	}
}
	...
	if (stsolve == STSOLVE_NONE || stsolve == STSOLVE_FERROMAGNETIC) {
		...
		if (!is_thermoelectric_mesh) {
			...
			if (!open_potential) {
				...
				CalculateElectricField_Charge_ThermoElectric_Kernel <<< (pMeshCUDA->n_e.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (pMeshCUDA->cuMesh);
			}
			else {
				...
				CalculateElectricField_Charge_ThermoElectric_OpenPotential_Kernel <<< (pMeshCUDA->n_e.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (pMeshCUDA->cuMesh, mesh_thermoelectric_net_current);
			}
		}
	}
	else {

		CalculateElectricField_Spin_withISHE_Kernel <<< (pMeshCUDA->n_e.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (pMeshCUDA->cuMesh);
	}
}
