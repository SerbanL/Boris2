...
__global__ void DemagAsymptoticDiag_setup_kernel(DemagAsymptoticDiagCUDA& da, double hx, double hy, double hz)
{
	int idx = threadIdx.x;

	if (idx == 0) {

		da.setup_aux(hx, hy, hz);
	}
}

__device__ void DemagAsymptoticDiagCUDA::setup_aux(double hx, double hy, double hz)
{
	double hx2 = hx * hx;
	double hy2 = hy * hy;
	double hz2 = hz * hz;
...
__host__ void DemagAsymptoticDiagCUDA::setup(double hx, double hy, double hz)
{
	DemagAsymptoticDiag_setup_kernel <<< 1, CUDATHREADS >>> (*this, hx, hy, hz);
}

__device__ double DemagAsymptoticDiagCUDA::AsymptoticLdia(double x, double y, double z)
{
...
__global__ void DemagAsymptoticOffDiag_setup_kernel(DemagAsymptoticOffDiagCUDA& da, double hx, double hy, double hz)
{
	int idx = threadIdx.x;
...
__device__ void DemagAsymptoticOffDiagCUDA::setup_aux(double hx, double hy, double hz)
{
	double hx2 = hx * hx;
	double hy2 = hy * hy;
	double hz2 = hz * hz;
...
__host__ void DemagAsymptoticOffDiagCUDA::setup(double hx, double hy, double hz)
{
	DemagAsymptoticOffDiag_setup_kernel <<< 1, CUDATHREADS >>> (*this, hx, hy, hz);
}

__device__ double DemagAsymptoticOffDiagCUDA::AsymptoticLodia(double x, double y, double z)
{
	double tx2 = 0, ty2 = 0, tz2 = 0;
	double R = 0, iR = 0;
	double R2 = 0, iR2 = 0;
