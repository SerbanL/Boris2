...
Simulation::Simulation(
	...
	int cudaDevice,
	std::string python_script_, std::vector<int> python_script_parallel_, int python_script_deletesource_) :
	err_hndl(this),
...

{
	...
	commands.insert(CMD_RUNSCRIPTNEWINSTANCE, CommandSpecifier(CMD_RUNSCRIPTNEWINSTANCE), "runscriptnewinstance");
	commands[CMD_RUNSCRIPTNEWINSTANCE].usage = "[tc0,0.5,0,1/tc]USAGE : <b>runscriptnewinstance</b> <i>(directory/)filename</i>";
	commands[CMD_RUNSCRIPTNEWINSTANCE].descr = "[tc0,0.5,0.5,1/tc]Execute a Python-scripted simulation (filename must be a Python script) in a new program instance. If directory not specified then default directory is used.";
	...
	commands[CMD_SETSTRESS].descr = "[tc0,0.5,0.5,1/tc]Set uniform mechanical stress (Pa) using polar coordinates. If mesh name not specified, this is set for all magnetic meshes - must have MElastic module added. Setting a uniform stress disables the elastodynamics solver.";
