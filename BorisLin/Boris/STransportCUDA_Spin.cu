			...
			if (((pTransport[idx_pri]->Get_STSolveType() == STSOLVE_FERROMAGNETIC || pTransport[idx_pri]->Get_STSolveType() == STSOLVE_FERROMAGNETIC_ATOM) && 
				...
				if ((CMBNDcontacts[idx1][idx2].IsPrimaryTop() && pTransport[idx_pri]->GInterface_Enabled()) ||
					...
					(*pV[idx_pri])()->set_cmbnd_continuousflux(
						size, *pV[idx_sec],
						(TransportCUDA_Spin_V_Funcs&)pTransport[idx_sec]->poisson_Spin_V,
						(TransportCUDA_Spin_V_Funcs&)pTransport[idx_pri]->poisson_Spin_V,
				...
				if ((CMBNDcontacts[idx1][idx2].IsPrimaryTop() && pTransport[idx_pri]->GInterface_Enabled()) ||
					...
					if (pTransport[idx_pri]->Get_STSolveType() == STSOLVE_FERROMAGNETIC || pTransport[idx_pri]->Get_STSolveType() == STSOLVE_FERROMAGNETIC_ATOM) {
						...
						(*pS[idx_pri])()->set_cmbnd_discontinuous(
							size, *pS[idx_sec],
							(TransportCUDA_Spin_S_Funcs&)pTransport[idx_sec]->poisson_Spin_S,
							(TransportCUDA_Spin_S_Funcs&)pTransport[idx_pri]->poisson_Spin_S,
