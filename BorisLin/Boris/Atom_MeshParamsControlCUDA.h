...
__device__ void ManagedAtom_MeshCUDA::update_parameters_mcoarse_spatial(int mcell_idx, MatPCUDA<PType, SType>& matp, PType& matp_value, MeshParam_List& ... params)
{
	if (matp.is_sdep()) {
		...
		cuReal3 position = pM1->cellidx_to_position(mcell_idx);
...
{
	if (matp.is_sdep()) matp_value = matp.get(pM1->cellidx_to_position(mcell_idx), *pcuaDiffEq->pstagetime);
}
...
__device__ void ManagedAtom_MeshCUDA::update_parameters_mcoarse_full(int mcell_idx, MatPCUDA<PType, SType>& matp, PType& matp_value)
{
	if (matp.is_sdep()) {
		...
		matp_value = matp.get(position, *pcuaDiffEq->pstagetime, (*pTemp)[position]);
	}
	else if (matp.is_tdep()) {
...
{
	if (matp.is_sdep()) matp_value = matp.get(pV->cellidx_to_position(ecell_idx), *pcuaDiffEq->pstagetime);
}
...
{
	if (matp.is_sdep()) matp_value = matp.get(pTemp->cellidx_to_position(tcell_idx), *pcuaDiffEq->pstagetime);
}
...
{
	if (matp.is_sdep()) matp_value = matp.get(pu_disp->cellidx_to_position(scell_idx), *pcuaDiffEq->pstagetime);
}
