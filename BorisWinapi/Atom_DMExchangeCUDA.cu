...
__global__ void Atom_DMExchangeCUDA_Cubic_UpdateField(ManagedAtom_MeshCUDA& cuaMesh, cuBReal& energy, bool do_reduction)
{
...
void Atom_DMExchangeCUDA::UpdateField(void)
{
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		if (paMeshCUDA->CurrentTimeStepSolved()) {
			...
			Atom_DMExchangeCUDA_Cubic_UpdateField << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, true);
		}
		else {

			Atom_DMExchangeCUDA_Cubic_UpdateField << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, false);
		}
	}
}
...
__global__ void Atom_DMExchangeCUDA_Cubic_GetEnergy(ManagedAtom_MeshCUDA& cuaMesh, cuBReal& energy, size_t& points_count, cuRect avRect)
{
	cuVEC_VC<cuReal3>& M1 = *cuaMesh.pM1;
...
__global__ void Atom_DMExchangeCUDA_Cubic_GetEnergy_Max(ManagedAtom_MeshCUDA& cuaMesh, cuBReal& energy, cuRect rectangle)
{
	cuVEC_VC<cuReal3>& M1 = *cuaMesh.pM1;
...
cuBReal Atom_DMExchangeCUDA::GetEnergyDensity(cuRect avRect)
{
	ZeroEnergy();
	...
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		Atom_DMExchangeCUDA_Cubic_GetEnergy << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, points_count, avRect);
	}
...
cuBReal Atom_DMExchangeCUDA::GetEnergy_Max(cuRect rectangle)
{
	ZeroEnergy();
	...
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		Atom_DMExchangeCUDA_Cubic_GetEnergy_Max << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, rectangle);
	}
...
__global__ void Atom_DMExchangeCUDA_Cubic_Compute_Exchange(ManagedAtom_MeshCUDA& cuaMesh, cuVEC<cuBReal>& exchange_displayVEC)
{
	cuVEC_VC<cuReal3>& M1 = *cuaMesh.pM1;
...
void Atom_DMExchangeCUDA::Compute_ExchangeCUDA(void)
{
	...
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		Atom_DMExchangeCUDA_Cubic_Compute_Exchange << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, exchange_displayVEC);
	}
}
