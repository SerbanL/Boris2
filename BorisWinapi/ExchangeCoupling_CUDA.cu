	...
	cuBReal& energy, bool do_reduction)
{
	...
	if (box_idx < box_sizes.dim()) {
		...
		if (M_pri.is_not_empty(cell1_idx) && M_pri.is_cmbnd(cell1_idx)) {
			...
			mesh_pri.update_parameters_mcoarse(cell1_idx, *mesh_pri.pA_AFM, A_AFM, *mesh_pri.pMs_AFM, Ms_AFM);
...
void Exch_6ngbr_NeuCUDA::CalculateExchangeCoupling(cu_obj<cuBReal>& energy)
{
