...
BError ManagedDiffEqAFMCUDA::set_pointers(DifferentialEquationAFMCUDA* pDiffEqCUDA)
{
	...
	if (set_gpu_value(psM1_2, pDiffEqCUDA->sM1_2.get_managed_object()) != cudaSuccess) error(BERROR_GPUERROR_CRIT);
	...
	if (set_gpu_value(psEval2_2, pDiffEqCUDA->sEval2_2.get_managed_object()) != cudaSuccess) error(BERROR_GPUERROR_CRIT);
