...
__device__ cuReal3 ManagedDiffEqFMCUDA::LLG(int idx)
{
	...
	return (-(cuBReal)GAMMA * grel / (1 + alpha * alpha)) * ((M[idx] ^ Heff[idx]) + alpha * ((M[idx] / Ms) ^ (M[idx] ^ Heff[idx])));
}

//Landau-Lifshitz-Gilbert equation but with no precession term and damping set to 1 : faster relaxation for static problems
__device__ cuReal3 ManagedDiffEqFMCUDA::LLGStatic(int idx)
{
	cuVEC_VC<cuReal3>& M = *pcuMesh->pM;
	cuVEC<cuReal3>& Heff = *pcuMesh->pHeff;
...
__device__ cuReal3 ManagedDiffEqFMCUDA::LLGSTT(int idx)
{
	...
	cuReal3 LLGSTT_Eval = (-(cuBReal)GAMMA * grel / (1 + alpha*alpha)) * ((M[idx] ^ Heff[idx]) + alpha * ((M[idx] / Ms) ^ (M[idx] ^ Heff[idx])));
...
__device__ cuReal3 ManagedDiffEqFMCUDA::LLB(int idx)
{
	//gamma = -mu0 * gamma_e = mu0 * g e / 2m_e = 2.212761569e5 m/As
	...
	return (-(cuBReal)GAMMA * grel / (1 + alpha_perp_red * alpha_perp_red)) * ((M[idx] ^ Heff[idx]) + alpha_perp_red * ((M[idx] / Mnorm) ^ (M[idx] ^ Heff[idx]))) +
		(cuBReal)GAMMA * grel * alpha_par_red * (M[idx] * (Heff[idx] + Hl)) * (M[idx] / Mnorm);
}

//------------------------------------------------------------------------------------------------------

__device__ cuReal3 ManagedDiffEqFMCUDA::LLBSTT(int idx)
{
	//gamma = -mu0 * gamma_e = mu0 * g e / 2m_e = 2.212761569e5 m/As
	...
	cuReal3 LLBSTT_Eval =
		(-(cuBReal)GAMMA * grel / (1 + alpha_perp_red * alpha_perp_red)) * ((M[idx] ^ Heff[idx]) + alpha_perp_red * ((M[idx] / Mnorm) ^ (M[idx] ^ Heff[idx]))) +
		(cuBReal)GAMMA * grel * alpha_par_red * (M[idx] * (Heff[idx] + Hl)) * (M[idx] / Mnorm);
