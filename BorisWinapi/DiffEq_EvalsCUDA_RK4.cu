...
__global__ void RunRK4_Step0_withReductions_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
...
__global__ void RunRK4_Step3_withReductions_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void DifferentialEquationCUDA::RunRK4(int step, bool calculate_mxh, bool calculate_dmdt)
{
	switch (step) {
