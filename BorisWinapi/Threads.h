...
//The methods in Threads then become available in the Owner, and function pointers can be used in Threads without having to pass the this pointer every time.
...
private:
	...
	vector<bool> blocking_call;

	vector<long> thread_active, thread_running;
...
protected:
	...
	enum THREAD_ { THREAD_GENERATENEW = 0, THREAD_LOOP, THREAD_HANDLEMESSAGE, THREAD_HANDLEMESSAGE2, THREAD_HANDLEMESSAGE3, THREAD_HANDLECLIENT, THREAD_NETWORK, THREAD_TIMEDCHECK };
...
private:
	...
	int run_on_thread(function<void()> runThis, int threadId);
...
template <class Owner> int Threads<Owner>::configure_threadId(int threadId = THREAD_GENERATENEW)
{
	...
	if (threadId >= (int)thread_active.size()) {
		...
		//generate a new thread id : find the first entry in thread_active which is not being used, or extend the vector
		int idx = 1;
...
template <class Owner> int Threads<Owner>::run_on_thread(function<void()> runThis, int threadId) 
{
...
int Threads<Owner>::infinite_loop_launch(void (Owner::*runThisMethod)(), int threadId)
{
	...
	function<void()> calling_method = [this, threadId, runThisMethod] {
	...
	//the mutex will be unlocked before this function returns
...
int Threads<Owner>::infinite_loop_launch(void (Owner::*runThisMethod)(PType...), PType... param, int threadId)
{
	...
	function<void()> calling_method = [this, param..., threadId, runThisMethod] {
...
int Threads<Owner>::delayed_call_launch(void (Owner::*runThisMethod)(), int delay_ms, int threadId)
{
	...
	function<void()> calling_method = [this, delay_ms, threadId, runThisMethod] {
...
int Threads<Owner>::delayed_call_launch(void (Owner::*runThisMethod)(PType...), PType... param, int delay_ms, int threadId)
{
	...
	function<void()> calling_method = [this, param..., delay_ms, threadId, runThisMethod] {
...
//example call, run HandleCommand(command) method only once, in its own asynchronous thread: threaded_call.single_call_launch<string>(&Simulation::HandleCommand, command)
