...
void DifferentialEquation::GenerateThermalField(void)
{
...
void DifferentialEquation::GenerateThermalField_and_Torque(void)
{
	//get a random number generator
...
DBL3 DifferentialEquation::SLLG(int idx)
{
...
DBL3 DifferentialEquation::SLLGSTT(int idx)
{
	...
	if (pMesh->Jc.linear_size()) {
		...
		DBL3 u = (pMesh->Jc.weighted_average(INT3(idx % pMesh->n.x, (idx / pMesh->n.x) % pMesh->n.y, idx / (pMesh->n.x*pMesh->n.y)), pMesh->h) * P * GMUB_2E) / (Ms * (1 + beta*beta));
...
DBL3 DifferentialEquation::SLLB(int idx)
{
	//gamma = -mu0 * gamma_e = mu0 * g e / 2m_e = 2.212761569e5 m/As
...
DBL3 DifferentialEquation::SLLBSTT(int idx)
{
	//gamma = -mu0 * gamma_e = mu0 * g e / 2m_e = 2.212761569e5 m/As
