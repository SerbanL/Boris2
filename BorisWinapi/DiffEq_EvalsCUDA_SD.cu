...
__global__ void RunSD_Start_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
	...
	if (idx < cuMesh.pM->linear_size()) {
		...
		if (cuMesh.pM->is_not_empty(idx)) {
			...
			if (!cuMesh.pM->is_skipcell(idx)) {
				...
				cuReal3 MxHeff = ((cuReal)GAMMA / 2) * ((*cuMesh.pM)[idx] ^ H);
...
__global__ void RunSD_BB_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;

	cuReal _delta_M_sq = 0.0;
	cuReal _delta_G_sq = 0.0;
	cuReal _delta_M_dot_delta_G = 0.0;
	...
	//the delta_... quantities in which we accumulate are cuReal (thus likely single precision depending on compilation flag SINGLEPRECISION)
...
__global__ void RunSD_Advance_withReductions_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (idx < cuMesh.pM->linear_size()) {
		...
		if (cuMesh.pM->is_not_empty(idx)) {
			...
			if (!cuMesh.pM->is_skipcell(idx)) {
				...
				cuReal mxh = cu_GetMagnitude(m ^ H) / Mnorm;
				...
				//only reduce for mxh and dmdt if grel is not zero (if it's zero this means magnetisation dynamics is disabled in this mesh)
...
__global__ void RunSD_Advance_withReduction_mxh_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (idx < cuMesh.pM->linear_size()) {
		...
		if (cuMesh.pM->is_not_empty(idx)) {
			...
			if (!cuMesh.pM->is_skipcell(idx)) {
				...
				//obtained maximum normalized torque term
				cuReal mxh = cu_GetMagnitude(m ^ H) / (*cuMesh.pM)[idx].norm();
...
__global__ void RunSD_Advance_withReduction_dmdt_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void RunSD_Advance_Kernel(ManagedDiffEqCUDA& cuDiffEq, ManagedMeshCUDA& cuMesh)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
