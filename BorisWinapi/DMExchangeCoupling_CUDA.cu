...
__global__ void CalculateDMExchangeCoupling_AFM_FM_kernel(
	ManagedMeshCUDA& mesh_sec, ManagedMeshCUDA& mesh_pri,
	CMBNDInfoCUDA& contact,
	cuBReal& energy, bool do_reduction)
{
...
void DMExchangeCUDA::CalculateExchangeCoupling(cu_obj<cuBReal>& energy)
{
	for (int contact_idx = 0; contact_idx < CMBNDcontacts.size(); contact_idx++) {
		...
		if (pContactingMeshes[idx_pri]->GetMeshType() == MESH_ANTIFERROMAGNETIC && pContactingMeshes[idx_sec]->GetMeshType() == MESH_ANTIFERROMAGNETIC) {
			...
			if (pMeshCUDA->CurrentTimeStepSolved()) {
				...
				CalculateDMExchangeCoupling_AFM_kernel << < (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (*pContactingManagedMeshes[idx_sec], *pContactingManagedMeshes[idx_pri], CMBNDcontactsCUDA[contact_idx], energy, true);
			}
			else {

				CalculateDMExchangeCoupling_AFM_kernel << < (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (*pContactingManagedMeshes[idx_sec], *pContactingManagedMeshes[idx_pri], CMBNDcontactsCUDA[contact_idx], energy, false);
			}
		}
			...
			if (pMeshCUDA->CurrentTimeStepSolved()) {
				...
				CalculateDMExchangeCoupling_AFM_FM_kernel << < (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (*pContactingManagedMeshes[idx_sec], *pContactingManagedMeshes[idx_pri], CMBNDcontactsCUDA[contact_idx], energy, true);
			}
			else {

				CalculateDMExchangeCoupling_AFM_FM_kernel << < (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (*pContactingManagedMeshes[idx_sec], *pContactingManagedMeshes[idx_pri], CMBNDcontactsCUDA[contact_idx], energy, false);
			}
		}
		else {
			...
			if (pMeshCUDA->CurrentTimeStepSolved()) {
				...
				CalculateDMExchangeCoupling_FM_kernel << < (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (*pContactingManagedMeshes[idx_sec], *pContactingManagedMeshes[idx_pri], CMBNDcontactsCUDA[contact_idx], energy, true);
			}
			else {

				CalculateDMExchangeCoupling_FM_kernel << < (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (*pContactingManagedMeshes[idx_sec], *pContactingManagedMeshes[idx_pri], CMBNDcontactsCUDA[contact_idx], energy, false);
			}
		}
	}
}
