...
double Atom_iDMExchange::GetEnergyDensity(Rect& avRect)
{
#if COMPILECUDA == 1
	if (pModuleCUDA) return dynamic_cast<Atom_iDMExchangeCUDA*>(pModuleCUDA)->GetEnergyDensity(avRect);
#endif
...
double Atom_iDMExchange::GetEnergy_Max(Rect& rectangle)
{
#if COMPILECUDA == 1
	if (pModuleCUDA) return dynamic_cast<Atom_iDMExchangeCUDA*>(pModuleCUDA)->GetEnergy_Max(rectangle);
#endif
...
void Atom_iDMExchange::Compute_Exchange(VEC<double>& displayVEC)
{
	...
	if (pModuleCUDA) {
		...
		dynamic_cast<Atom_iDMExchangeCUDA*>(pModuleCUDA)->Compute_Exchange(displayVEC);
		return;
	}
#endif
