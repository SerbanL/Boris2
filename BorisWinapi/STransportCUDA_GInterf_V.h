...
public:
	...
	__device__ cuReal A_func(int cell1_idx, int cell2_idx, cuReal3 relpos_m1, cuReal3 shift, cuReal3 stencil, TransportCUDA_Spin_V_Funcs& trans_sec, TransportCUDA_Spin_V_Funcs& trans_pri) const
	{
		...
		cuVEC_VC<cuReal>& elC_sec = *(trans_sec.pcuMesh->pelC);
		cuVEC_VC<cuReal>& elC_pri = *(trans_pri.pcuMesh->pelC);

		cuVEC_VC<cuReal3>& M_sec = *(trans_sec.pcuMesh->pM);
		cuVEC_VC<cuReal3>& M_pri = *(trans_pri.pcuMesh->pM);

		cuReal De_sec = *(trans_sec.pcuMesh->pDe);
		cuReal De_pri = *(trans_pri.pcuMesh->pDe);
		...
		cuReal elC_pri_val = 1.5 * elC_pri[cell1_idx] - 0.5 * elC_pri[cell2_idx];
		cuReal elC_sec_val = 1.5 * elC_sec.weighted_average(relpos_m1, stencil) - 0.5 * elC_sec.weighted_average(relpos_m1 + shift, stencil);
		...
		if (M_pri.linear_size()) {
			...
			cuReal3 dVs = (De_pri * S_pri_val / elC_pri_val - De_sec * S_sec_val / elC_sec_val) / (cuReal)MUB_E;
	...
	__device__ cuReal B_func(int cell1_idx, int cell2_idx, cuReal3 relpos_m1, cuReal3 shift, cuReal3 stencil, TransportCUDA_Spin_V_Funcs& trans_sec, TransportCUDA_Spin_V_Funcs& trans_pri) const
	{
		//the shift value comes from CMBNDInfo and is used to find positions in secondary mesh. If negative then secondary mesh is on -ve side (bottom).
		bool primary_top = !(shift >= cuReal3(0));
