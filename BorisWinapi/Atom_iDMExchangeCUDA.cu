...
#if defined(MODULE_IDMEXCHANGE) && ATOMISTIC == 1
...
__global__ void Atom_iDMExchangeCUDA_Cubic_UpdateField(ManagedAtom_MeshCUDA& cuaMesh, cuBReal& energy, bool do_reduction)
{
	...
	if (idx < Heff1.linear_size()) {
		...
		if (M1.is_not_empty(idx)) {
			...
			Hexch = (J * M1.ngbr_dirsum(idx) + D * M1.zanisotropic_ngbr_dirsum(idx)) / (MUB_MU0*mu_s);
...
void Atom_iDMExchangeCUDA::UpdateField(void)
{
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		if (paMeshCUDA->CurrentTimeStepSolved()) {
			...
			Atom_iDMExchangeCUDA_Cubic_UpdateField << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, true);
		}
		else {

			Atom_iDMExchangeCUDA_Cubic_UpdateField << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, false);
		}
	}
}
...
__global__ void Atom_iDMExchangeCUDA_Cubic_GetEnergy(ManagedAtom_MeshCUDA& cuaMesh, cuBReal& energy, size_t& points_count, cuRect avRect)
{
	cuVEC_VC<cuReal3>& M1 = *cuaMesh.pM1;
...
__global__ void Atom_iDMExchangeCUDA_Cubic_GetEnergy_Max(ManagedAtom_MeshCUDA& cuaMesh, cuBReal& energy, cuRect rectangle)
{
	cuVEC_VC<cuReal3>& M1 = *cuaMesh.pM1;
...
cuBReal Atom_iDMExchangeCUDA::GetEnergyDensity(cuRect avRect)
{
	ZeroEnergy();
	...
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		Atom_iDMExchangeCUDA_Cubic_GetEnergy << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, points_count, avRect);
	}
...
cuBReal Atom_iDMExchangeCUDA::GetEnergy_Max(cuRect rectangle)
{
	ZeroEnergy();
	...
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		Atom_iDMExchangeCUDA_Cubic_GetEnergy_Max << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, energy, rectangle);
	}
...
__global__ void Atom_iDMExchangeCUDA_Cubic_Compute_Exchange(ManagedAtom_MeshCUDA& cuaMesh, cuVEC<cuBReal>& exchange_displayVEC)
{
	cuVEC_VC<cuReal3>& M1 = *cuaMesh.pM1;
...
void Atom_iDMExchangeCUDA::Compute_ExchangeCUDA(void)
{
	...
	if (paMeshCUDA->GetMeshType() == MESH_ATOM_CUBIC) {
		...
		Atom_iDMExchangeCUDA_Cubic_Compute_Exchange << < (paMeshCUDA->n.dim() + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> > (paMeshCUDA->cuaMesh, exchange_displayVEC);
	}
}
