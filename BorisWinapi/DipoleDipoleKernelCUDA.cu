...
#if defined(MODULE_ATOM_DIPOLEDIPOLE)
...
//N = (N.x/2 + 1, N.y, 1)
__global__ void cu_DD_ConvProd_2D(cuVEC<cuReal3>& Kdiag, cuVEC<cuBReal>& K2D_odiag, cuBComplex* cuSx, cuBComplex* cuSy, cuBComplex* cuSz, cuSZ3& N)
{
...
__global__ void cu_DD_ConvProd_2D_transpose_xy(cuVEC<cuReal3>& Kdiag, cuVEC<cuBReal>& K2D_odiag, cuBComplex* cuSx, cuBComplex* cuSy, cuBComplex* cuSz, cuSZ3& N)
{
	//above N.y/2 use kernel symmetries to recover kernel values
	//diagonal components are even about the N.y/2 point
	//off-diagonal values are odd about the N.y/2 point
...
//N = (N.x/2 + 1, N.y, N.z)
__global__ void cu_DD_ConvProd_3D_transpose_xy(cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag, cuBComplex* cuSx, cuBComplex* cuSy, cuBComplex* cuSz, cuSZ3& N)
{
...
//N = (N.x/2 + 1, N.y, 4)
//xy is transposed
__global__ void cu_DD_ConvProd_q2D_4_transpose_xy(cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag, cuBComplex* cuSx, cuBComplex* cuSy, cuBComplex* cuSz, cuSZ3& N)
{
	//above N.z/2 and N.y/2 use kernel symmetries to recover kernel values
	//diagonal components are even about the N.z/2 and N.y/2 points
	//Kxy is even about N.z/2 and odd about N.y/2
...
//N = (N.x/2 + 1, N.y, 8)
//xy is transposed
__global__ void cu_DD_ConvProd_q2D_8_transpose_xy(cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag, cuBComplex* cuSx, cuBComplex* cuSy, cuBComplex* cuSz, cuSZ3& N)
{
	//above N.z/2 and N.y/2 use kernel symmetries to recover kernel values
	//diagonal components are even about the N.z/2 and N.y/2 points
	//Kxy is even about N.z/2 and odd about N.y/2
...
//N = (N.x/2 + 1, N.y, 16)
//xy is transposed
__global__ void cu_DD_ConvProd_q2D_16_transpose_xy(cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag, cuBComplex* cuSx, cuBComplex* cuSy, cuBComplex* cuSz, cuSZ3& N)
{
	//above N.z/2 and N.y/2 use kernel symmetries to recover kernel values
	//diagonal components are even about the N.z/2 and N.y/2 points
	//Kxy is even about N.z/2 and odd about N.y/2
...
//N = (N.x/2 + 1, N.y, 32)
//xy is transposed
__global__ void cu_DD_ConvProd_q2D_32_transpose_xy(cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag, cuBComplex* cuSx, cuBComplex* cuSy, cuBComplex* cuSz, cuSZ3& N)
{
	//above N.z/2 and N.y/2 use kernel symmetries to recover kernel values
	//diagonal components are even about the N.z/2 and N.y/2 points
	//Kxy is even about N.z/2 and odd about N.y/2
...
void DipoleDipoleKernelCUDA::KernelMultiplication_2D(void)
{
	if (transpose_xy) {
		...
		cu_DD_ConvProd_2D_transpose_xy <<< ((N.x / 2 + 1)*N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, K2D_odiag, cuS_x, cuS_y, cuS_z, cuN);
	}
	else {

		cu_DD_ConvProd_2D <<< ((N.x / 2 + 1)*N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, K2D_odiag, cuS_x, cuS_y, cuS_z, cuN);
	}
}
...
void DipoleDipoleKernelCUDA::KernelMultiplication_3D(void)
{
	...
	cu_DD_ConvProd_3D_transpose_xy <<< ((N.x / 2 + 1)*N.y*N.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuN);
}
...
void DipoleDipoleKernelCUDA::KernelMultiplication_q2D(int q2D_level)
{
	...
	switch (q2D_level)
	{
		//N.z = 4, n.z = 2
	case 4:
		cu_DD_ConvProd_q2D_4_transpose_xy <<< ((N.x / 2 + 1)*N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuN);
		break;

		//N.z = 8, n.z = 3, 4
	case 8:
		cu_DD_ConvProd_q2D_8_transpose_xy <<< ((N.x / 2 + 1)*N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuN);
		break;

		//N.z = 16, n.z = 5, 6, 7, 8
	case 16:
		cu_DD_ConvProd_q2D_16_transpose_xy <<< ((N.x / 2 + 1)*N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuN);
		break;

		//N.z = 32, n.z = 9, 10, ..., 16
	case 32:
		cu_DD_ConvProd_q2D_32_transpose_xy <<< ((N.x / 2 + 1)*N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuN);
		break;
